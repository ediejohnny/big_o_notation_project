<div class="space-y-4">
    <div class="grid md:grid-cols-2 gap-3">
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Array de números</label>
            <input type="text" wire:model.live.debounce.500ms="input" placeholder="2,7,11,15,3,6,4"
        ...
        <div>
            ...
            <input type="number" wire:model.live.debounce.500ms="target" placeholder="20"
                   class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
    </div>
    ...
    <button wire:click="run" class="w-full px-4 py-2 bg-pink-600 hover:bg-pink-700 text-white font-medium rounded-lg transition-colors">
        Buscar três números (Força Bruta)
    </button>

    @if ($result !== null)
        <div class="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
            <div class="text-sm font-medium text-green-800 dark:text-green-300">✅ Encontrado</div>
            <div class="mt-1 font-mono text-green-900 dark:text-green-200">{{ $result[0] }} + {{ $result[1] }} + {{ $result[2] }} = {{ $target }}</div>
        </div>
    ...
    @if ($combinations > 0 || $additions > 0)
        <div class="grid grid-cols-3 gap-3">
            <div class="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                <div class="text-xs font-medium text-blue-600 dark:text-blue-400">Combinações</div>
                <div class="text-xl font-bold text-blue-900 dark:text-blue-100">{{ $combinations }}</div>
            </div>
